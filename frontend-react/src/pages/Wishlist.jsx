  ...
  if (wishlistItems.length === 0) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, gap: '12px' }}
        ...
        <span className="material-symbols-outlined" style={{ fontSize: '64px' }}>favorite</span>
        <p className="font-headline-sm text-headline-sm">Your wishlist is empty</p>
        <p className="font-label-sm text-label-sm">Heart a game to save it here</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column', padding: '24px 32px', gap: '16px' }}>
      <h2 className="font-headline-md text-headline-md font-bold text-on-background" style={{ flexShrink: 0 }}>
        Wishlist ({wishlistItems.length} {wishlistItems.length === 1 ? 'game' : 'games'})
      </h2>
          ...
          return (
            <div key={game.id}
              ...
              <div style={{ position: 'relative', overflow: 'hidden', flexShrink: 0 }}>
                <img alt={game.title} className="w-full h-auto block" src={game.image} />
                ...
                <button onClick={() => toggleWishlist(game)}
                  className="absolute top-2 left-2 w-7 h-7 bg-black/50 backdrop-blur-md rounded-full flex items-center justify-center text-red-400 transition-colors">
                  <span className="material-symbols-outlined fill" style={{ fontSize: '16px' }}>favorite</span>
                </button>
              </div>
